...
<Callout type="info">
  **Using SDK 3.0?** The `ApiGatewayClient` wraps these endpoints with typed errors, balance tracking, and Bearer auth out of the box. See [Client SDK 3.0 → API Gateway](/docs-v2/sdk-v3/api-gateway).
</Callout>
...
<Tabs items={['JavaScript/Fetch', 'cURL', 'Python']}>
  <Tabs.Tab>
    ...
    fetch('https://YOUR_DOMAIN/api/v1/api-gateway/your-provider-id', {
      method: 'POST',
      body: JSON.stringify({
    ...
    fetch('https://YOUR_DOMAIN/api/v1/api-gateway/your-provider-id/profile/settings', {
      method: 'POST',
      body: JSON.stringify({
        // Your request data here
      })
    ...
    fetch('https://YOUR_DOMAIN/api/v1/api-gateway/your-provider-id/123/orders', {
      method: 'GET'
    });
  ...
  <Tabs.Tab>
    ```bash
    # Request to base endpoint
    curl -X POST https://YOUR_DOMAIN/api/v1/api-gateway/your-provider-id \
      -H "Content-Type: application/json" \
      -d '{"key": "value"}'

    # Request with additional path - append to provider URL
    curl -X POST https://YOUR_DOMAIN/api/v1/api-gateway/your-provider-id/profile/settings \
      -H "Content-Type: application/json" \
      -d '{"key": "value"}'

    # GET request with path
    curl -X GET https://YOUR_DOMAIN/api/v1/api-gateway/your-provider-id/123/orders
    ```
  </Tabs.Tab>
  <Tabs.Tab>
    ```python
    ...
    response = requests.post(
        'https://YOUR_DOMAIN/api/v1/api-gateway/your-provider-id',
        json={'key': 'value'}
    )

    # Request with additional path - append to provider URL
    response = requests.post(
        'https://YOUR_DOMAIN/api/v1/api-gateway/your-provider-id/profile/settings',
        json={'key': 'value'}
    )

    # GET request with path
    response = requests.get(
        'https://YOUR_DOMAIN/api/v1/api-gateway/your-provider-id/123/orders'
    )
    ```
  </Tabs.Tab>
</Tabs>
...
// Chat with GPT-4
fetch('https://YOUR_DOMAIN/api/v1/api-gateway/openai-provider/chat/completions', {
  method: 'POST',
  body: JSON.stringify({
    model: 'gpt-4',
    messages: [...]
...
// Image generation
fetch('https://YOUR_DOMAIN/api/v1/api-gateway/openai-provider/images/generations', {
  method: 'POST',
  body: JSON.stringify({
...
// File operations
fetch('https://YOUR_DOMAIN/api/v1/api-gateway/openai-provider/files')
...
<Cards>
  ...
  />
  <Cards.Card
    title="SDK v3 — ApiGatewayClient.call()"
    href="/docs-v2/sdk-v3/api-gateway"
  />
  <Cards.Card
    title="SDK v2 — paywall.makeRequest()"
    href="/docs-v2/sdk-v2/client-side/make-request"
  />
</Cards>
