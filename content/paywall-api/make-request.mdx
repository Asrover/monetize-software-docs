...
<Callout type="warning">
  This API only available when paywall is in client mode and has tokenization
  enabled. [Learn more](/docs-v2/paywall/tokenization) about paywall
  tokenization.
</Callout>
...
<Callout type="info">
  **Prerequisites:** You must have an [API provider
  created](/docs-v2/api-provider/create-api-provider) before making requests.
  API providers handle the connection to external APIs and token deduction.
</Callout>
...
When you need to upload files, call `paywall.makeRequest()` in FormData mode. Do not set the `Content-Type` header yourself; the browser will set the correct boundary.

```javascript
// Single file upload with additional fields (if needed)
const file = document.querySelector("#file-input").files?.[0];
if (!file) {
  throw new Error("Select a file before sending");
}
...
const response = await paywall.makeRequest(
  "https://onlineapp.pro/api/v1/api-gateway/999?paywall_id=100",
  {
    ...
    body: {
      file, // File object (e.g., from <input type="file" />)
      // You can add extra fields as simple key/value pairs:
      // userId: '123',
      // metadata: JSON.stringify({ foo: 'bar' })
    },
  },
  undefined,
  true // enable FormData mode
);
...
function UploadExample() {
  ...
  const onUpload = async () => {
    ...
    if (!file) return alert("Select a file before sending");

    const res = await paywall.makeRequest(
      "https://onlineapp.pro/api/v1/api-gateway/999?paywall_id=100",
      {
        method: "POST",
        body: { file },
...
```javascript
const requestUrl =
  "https://onlineapp.pro/api/v1/api-gateway/999?paywall_id=197&test_mode=1";
let answer = "";
...
<Callout type="warning">
  **Authentication Required:** If the user is not authenticated, these methods
  return `Unauthorized` error.
</Callout>

<Callout type="warning">
  **Insufficient Tokens:** If users' queries have run out, it will return a
  `not-enough-queries` error.
</Callout>
...
<Table>
  ...
  <tbody>
    ...
    <Table.Tr>
      ...
      <Table.Td>
        Call `paywall.renew()` to renew subscription and deposit tokens.
      </Table.Td>
    </Table.Tr>
  </tbody>
</Table>
...
<Cards>
  ...
  />
  <Cards.Card title="Handle Events" href="/docs-v2/paywall-api/events" />
</Cards>
