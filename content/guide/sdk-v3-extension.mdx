...
<Callout type="warning">
  **Tradeoff — take this one seriously.** Each surface runs its own `AuthClient`/`BillingClient`: simultaneously-open surfaces fetch bootstrap independently and refresh the token on their own schedule. `chrome.storage.onChanged` keeps them in sync, but it is asynchronous — two surfaces can still hit `/auth/refresh` with the same rotating refresh token, and when that race lands outside the server's reuse window, the auth backend revokes the **whole session family**. We've seen this in production: a paying user whose session died this way saw the paywall again, and every purchase click failed with a 401. The SDK softens the blow — on a 401 it forces one token refresh and replays the request, and `PaywallUI` reopens the sign-in gate with the checkout pending instead of a dead-end error (see [Error codes](/docs-v2/sdk-v3/errors)) — but the user still has to sign in again, and a guest/anonymous purchase may not be recoverable at all. If more than one privileged page can realistically be open at the same time (e.g. several download/progress tabs), or you need content scripts / the SW to read billing, use the single-instance offscreen architecture below: one session owner, no rotation race at all.
</Callout>
...
You do **not** need the `"identity"` permission for OAuth — the SDK uses the web auth flow (`window.open` + a callback page), not `chrome.identity`, so providers keep their web redirect URLs and the manifest stays minimal. Add `"content_scripts"` only if you render the paywall on third-party sites.
...
installRouter({
  ...
  // Your custom_domain — the same value PaywallUI and startOffscreenServer get.
...
<Callout type="warning">
  **Pass `apiOrigin` if your paywall lives in the toolbar action popup.** OAuth returns the auth code by `postMessage` to the window that opened the provider — and Chrome destroys an action popup as soon as that window takes focus. Whether it does is decided by the OS window manager, so the same extension signs in fine on one machine and silently fails on another: the popup vanishes, nothing happens, and opening DevTools appears to fix it (DevTools keeps the popup alive).

  With `apiOrigin` set, the service worker watches for the provider's redirect landing on your callback page, reads the code out of the URL, and lets the offscreen document — which holds the PKCE verifier — finish the sign-in without the popup. **No extra permission is needed**: the URL is already visible through the `host_permissions` entry for that origin.

  If the sign-in was gating a checkout, that continues as well: offscreen creates it the moment the session lands and the worker sends the same tab to the payment page, so the user goes from the provider straight to paying instead of reopening the extension to click buy again.

  It cannot help if the offscreen document itself died mid-flow (extension reload, browser restart), and provider *errors* arrive in the URL fragment, which a worker cannot read — those still end up reported as a cancellation. A **side panel** or full-page tab is immune to all of this and remains the sturdiest surface.
</Callout>
...
<Cards>
  ...
  <Cards.Card title="Authentication" href="/docs-v2/sdk-v3/auth" description="OAuth in extensions via the web flow (no identity permission)" />
</Cards>
