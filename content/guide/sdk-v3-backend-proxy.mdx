...
<Callout type="warning">
  **If you read `country` / `tier` from this response, add `X-Client-Ip`.** This call originates from your server, so without the header the geo fields describe *your datacenter*, not the user — every user comes back with the same country. Forward the end-user's IP (`req.ip` / the first entry of your own `X-Forwarded-For`) as `X-Client-Ip`; passing `X-Forwarded-For` upstream has no effect, our load balancer rewrites it.
</Callout>
