...
<Callout type="info">
  **Two layers.** The SDK exposes a typed customer-portal API on `BillingClient` (`listPurchases`, `cancelSubscription`, `getCustomerPortalUrl`) and you build the screens. For Stripe/Paddle/Chargebee/Polar customers we also have an acquirer-hosted portal — open it via `getCustomerPortalUrl()` when you don't need a custom UI.
</Callout>
...
This skips the `has_active_subscription` check (otherwise `open()` is suppressed for an already-subscribed user — the modal doesn't mount and the SDK emits `purchase_completed{restored: true}`) and passes `ignoreActivePurchase: true` to `/start-checkout`. The acquirer creates a new subscription; the old one cancels at period end.
