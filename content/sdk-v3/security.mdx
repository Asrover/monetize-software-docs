...
<Table>
  ...
  <tbody>
    ...
    <Table.Tr>
      ...
      <Table.Td>**Throws `apikey_in_browser`** in browsers (`window.document` detection) — a naive client integration fails to construct instead of leaking the key. `allowInsecureBrowserUsage: true` downgrades it to `console.error` for e2e only.</Table.Td>
    </Table.Tr>
    <Table.Tr>
