...
```
X-Api-Key: sk_live_...        (or Authorization: Bearer ...)
X-Client-Ip: 203.0.113.42     (optional — see below)
```

<Callout type="warning">
  **Calling from your backend? Send `X-Client-Ip`.** `country` and `tier` are resolved from the IP of the request that reaches us. When your server calls this endpoint on behalf of a user — even with a Bearer token the user's browser obtained — that IP is *your server's*, so every user comes back with your datacenter's country (and its tier). Pass the end-user's IP in `X-Client-Ip` and we geo-resolve that instead. `X-Forwarded-For` does **not** work for this: our load balancer rewrites the proxy chain, so your value is ignored.

  The header is a hint, and it only affects the `country`, `tier` and `countryMatch` fields of this response — targeting and pricing on the paywall itself are resolved separately, from the browser's own request.
</Callout>
...
<Callout type="info">
  **Verifying a user's token on your own backend.** This is how a third-party backend (e.g. an AI proxy) authenticates a paywall user without building its own auth: take the `Authorization: Bearer` the browser obtained from `AuthClient.getAccessToken()`, forward it here, and read the verified `user.id` from the response (`401` ⇒ invalid/expired — reject). Use that `user.id` to debit via [`/balances`](#post-apiv1paywallidbalances) with your `X-Api-Key` — the Bearer itself can't change a balance. Full walkthrough: [Backend AI Proxy guide](/docs-v2/guide/sdk-v3-backend-proxy). We don't expose a JWKS endpoint — calling this endpoint is the supported way to validate a session.
</Callout>
...
Returns a one-time URL to the acquirer-hosted customer portal (Stripe / Paddle / Chargebee / Polar / Overpay). Freemius does not provide a hosted portal — this endpoint returns `403` with `{ isTest }` instead. For Polar the access token needs the `customer_sessions:write` scope.
...
<Cards>
  ...
  <Cards.Card title="Headless / server-side" href="/docs-v2/sdk-v3/headless-server" description="apiKey + identity pattern, creditTokens/debitTokens, webhook handler, per-user storage" />
