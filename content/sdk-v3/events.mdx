...
While an [A/B experiment](/docs-v2/sdk-v3/experiments) is assigned, every event in this channel is automatically enriched with `experiment_id` + `variant` props — that's what powers the per-variant conversion report in the dashboard, so keep analytics enabled during experiments. When `open({ title })` overrides the paywall heading, `paywall_viewed` additionally carries `custom_title: true`.
