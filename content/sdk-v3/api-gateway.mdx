...
// A visitor without a Bearer (no AuthClient, or an AuthClient whose visitor is
// signed out) is answered `[]` straight from memory — `/balances` would reply
// 401, so the SDK does not spend the round-trip. Their snapshot is `[]`, never
// null: distinguish "loading" from "no quotas" by your own auth state, not by
// a null check.
...
<Cards>
  ...
  <Cards.Card
    ...
    href="/docs-v2/sdk-v3/headless-server#token-credit-and-debit"
    description="From your own backend — call providers directly and credit/debit balances via creditTokens/debitTokens"
  />
</Cards>
