...
Attribution is attached automatically: the request carries the stable visitor id and, when an [A/B experiment](/docs-v2/sdk-v3/experiments) is assigned, the experiment id + variant — this is what links the resulting purchase back to the paywall view/variant in analytics. Nothing to configure.
...
A visitor without a Bearer — no `AuthClient` wired up, or one whose visitor is
signed out — gets `[]` from memory and no network request at all: `/balances`
answers 401 to a request with no token, so the round-trip buys nothing. Their
`getCachedBalances()` is therefore `[]` rather than `null`, and it is safe to
call `getBalances()` (or `paywall.preload()`, which warms balances too) as often
as your UI needs.
...
```ts
billing.getExperimentAssignment(): { experimentId: string; variant: string } | null;
```

Sticky [A/B experiment](/docs-v2/sdk-v3/experiments) assignment of this device, read from the materialized bootstrap. `null` — no running experiment, or the bootstrap has not loaded yet. When a variant is assigned, `bootstrap.prices` / `getPrices()` already return that variant's prices — this getter exists for mirroring the assignment into your own analytics or host UI, not for branching pricing logic by hand.
