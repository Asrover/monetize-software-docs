...
The `paywall.signOut()` function logs out the current user from the paywall system, clearing their authentication session and any stored user data.
...
```typescript
paywall.signOut(): Promise<Response>
```

### Return Value

Returns a `Promise` that resolves with the server response when the sign-out request completes.
...
window.addEventListener('message', function (event) {
  if (
    ...
    ['https://appbox.space'].includes(event.origin)
  ) {
...
<Cards>
  <Cards.Card 
    ...
    href="/docs-v2/sdk-v2/client-side/events"
  />
  <Cards.Card 
    title="Open Paywall" 
