...
Modify user token balances in test paywalls for testing and development purposes.

<Callout type="warning">
  **Test Mode Only:** This API only works with test payment processors and is intended for development and testing purposes only.
</Callout>

<Callout type="info">
  **Tokenized Paywalls Only:** This feature only works with tokenized paywalls. [Learn more](/docs-v2/paywall/tokenization) about paywall tokenization.
</Callout>

## Overview

The modify balances API allows you to withdraw or deposit token balances for users in test paywalls. This is useful for:
...
```
POST https://appbox.space/api/v1/test/withdraw-balances
```
...
| `tokens`     | number | No       | Sets balances to specified value. If not provided, balances will be set to zero |
...
const setUserBalance = async (paywallId, userId, tokens) => {
  try {
    const response = await fetch('https://appbox.space/api/v1/test/withdraw-balances', {
      method: 'POST',
      headers: {
