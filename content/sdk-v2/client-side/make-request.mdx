...
```
https://appbox.space/api/v1/api-gateway/{API_PROVIDER_ID}?paywall_id={PAYWALL_ID}
```

<Callout type="info">
  If you are using a custom domain, make sure to replace "appbox.space" with
  your domain in the request URL, otherwise authentication will not work.
</Callout>
...
const response = await paywall.makeRequest(
  "https://appbox.space/api/v1/api-gateway/999?paywall_id=100",
  {
    method: "POST",
    body: JSON.stringify({
...
```javascript
const requestUrl =
  "https://appbox.space/api/v1/api-gateway/999?paywall_id=197&test_mode=1";
let answer = "";
...
<Cards>
  ...
  />
  <Cards.Card title="Handle Events" href="/docs-v2/sdk-v2/client-side/events" />
</Cards>
