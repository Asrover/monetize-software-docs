...
<Table>
  ...
  <tbody>
    <Table.Tr>
      ...
      <Table.Td>Object containing key-value pairs of metadata to store</Table.Td>
    </Table.Tr>
  </tbody>
</Table>
...
const trackUserJourney = () => {
  const metadata = {
    ...
    time_on_site: Math.floor((Date.now() - sessionStorage.getItem('session_start')) / 1000).toString(),
    ...
    if (metadata && Object.keys(metadata).length > 0) {
      ...
      Object.entries(metadata).forEach(([key, value]) => {
        if (typeof value === 'string' && value.length <= 100) {
          ...
          console.warn(`Invalid metadata for key ${key}: must be string with max 100 characters`);
        }
      });
