...
Use the [Get Prices API](/docs-v2/sdk-v2/server-side/get-prices) to retrieve available price IDs.

</Steps>

<Callout type="warning">
  **Never hardcode the price ID.** Price IDs are dynamic — they change when you
  edit pricing, switch payment processors, or recreate a plan. Always fetch the
  current `priceId` at runtime from the [Get Prices
  API](/docs-v2/sdk-v2/server-side/get-prices) before starting a checkout. A
  hardcoded ID will eventually point to a stale or non-existent price and the
  checkout will fail with a `404 Not Found`.
</Callout>
...
<Cards>
  <Cards.Card title="Get Prices" href="/docs-v2/sdk-v2/server-side/get-prices" />
